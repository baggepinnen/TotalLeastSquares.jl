...
* A::AbstractArray{<:Real}: Vector or Matrix (n x p) containing regressors 
...
* outlier::Real: Amount of outliers, between 0 <= outlier <= 0.5. A valid definition of h overwrites an outlier definition!
* N::Int: Number of initial p-subsets, defaults to 500 (minimum is 10). This is the main parameter regarding speed, to estimate a reasonable N, the probability of having at least one outlier free p-subset is given by: 1-(1-(1-outlier)^p)^N > 0. If verbose = true information about this gets printed.
...
```
"""
function flts(A::AbstractArray{<:Real}, y::Vector{<:Real}; h::Int = 0, outlier::Real = -1, N::Int = 500, maxiter::Int = 100, ΔQmin::Real = 0.0001, return_set::Bool = false, verbose::Bool = false)
    # Input checking
    n = length(y)
    ...
    if (round(0.5(n + p + 1)) <= h <= n)
        h = h
        verbose && @info "h was set to: h = $h. Breakdown point is at ~$(round(100 - h / n * 100))% outlier"
    elseif (0.0 <= outlier <= 0.5)
        h = Int(round((1-outlier)*n))
        verbose && @info "h was set to: h = $h. Breakdown point is at ~$(round(100 - h / n * 100))% outlier"
    else   
        h = Int(round(0.5(n + p + 1)))
        verbose && @info "h was set to default: h = $h. Breakdown point is at ~$(round(100 - h / n * 100))% outlier"
    end
